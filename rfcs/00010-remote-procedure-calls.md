...
The HTTP request path starts with `/xrpc/`, followed by an [NSID](./00009-namespace-identifiers.md). Paths must always be top-level, not below a prefix. The NSID maps to the `id` field in the associated Lexicon.
...
### Inter-Service Authentication (JWT)[#](https://atproto.com/specs/xrpc#inter-service-authentication-jwt)
...
- `alg` header field: indicates the signing key type (see [Cryptography](https://atproto.com/specs/cryptography))
...
## Service Proxying
_Note: This is a new protocol feature, and more details are likely to be specified over time._
...
`atproto-accept-labelers`: used by clients to request labels from specific labelers to be included and applied in the response. See [Label](https://atproto.com/specs/label) specification for details.
